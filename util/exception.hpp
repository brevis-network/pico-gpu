...
class sppark_error : public std::runtime_error
{
  ...
  inline std::string fmt_errno(int errnum, const char* fmt, Types... args)
  {
    ...
    if (errstr != errmsg) strncpy(errmsg, errstr, ERRLEN - 1);
#else
...
public:
  sppark_error(int err, const std::string& reason) : std::runtime_error{reason} { _code = err; }
  sppark_error(int err, const char* msg = "") : std::runtime_error{fmt_errno(err, "%s", msg)} { _code = err; }
  template <typename... Types>
