...
public:
  ...
  static const uint32_t TOP_BITS = 4;
  ...
  static inline constexpr uint32_t to_monty(uint32_t x) { return (((uint64_t)x << MONTY_BITS) % MOD); }
  ...
  static inline bb31_t from_canonical_u32(uint32_t x) { return bb31_t(to_monty(x)); }

  static inline bb31_t from_canonical_u16(uint16_t x) { return from_canonical_u32((uint32_t)x); }

  static inline bb31_t from_canonical_u8(uint8_t x) { return from_canonical_u32((uint32_t)x); }
  ...
  inline bb31_t operator-() { return zero() - *this; }
...
private:
  ...
  inline bb31_t& mul(const bb31_t b)
  {
    ...
    asm("mul.lo.u32 %0, %2, %3; mul.hi.u32 %1, %2, %3;" : "=r"(tmp[0]), "=r"(tmp[1]) : "r"(val), "r"(b.val));
...
public:
  ...
  static inline bb31_t dot_product(const bb31_t a[T], const bb31_t b[T])
  {
    ...
    asm("mul.lo.u32 %0, %2, %3; mul.hi.u32 %1, %2, %3;" : "=r"(acc[0]), "=r"(acc[1]) : "r"(*a[0]), "r"(*b[0]));
    if ((T & 1) == 0) {
    ...
    asm("mad.lo.cc.u32 %0, %2, %3, %0; madc.hi.u32 %1, %2, %3, %1;" : "+r"(acc[0]), "+r"(acc[1]) : "r"(red), "r"(MOD));
    final_sub(acc[1]);
  ...
  template <size_t T>
  static inline bb31_t dot_product(bb31_t a0, bb31_t b0, const bb31_t a[T - 1], const bb31_t* b, size_t stride_b = 1)
  {
    uint32_t acc[2];
    size_t i = 0;

    asm("mul.lo.u32 %0, %2, %3; mul.hi.u32 %1, %2, %3;" : "=r"(acc[0]), "=r"(acc[1]) : "r"(*a0), "r"(*b0));
    if ((T & 1) == 0) {
      asm("mad.lo.cc.u32 %0, %2, %3, %0; madc.hi.u32 %1, %2, %3, %1;"
          : "+r"(acc[0]), "+r"(acc[1])
          : "r"(*a[i]), "r"(*b[0]));
...
private:
  static inline bb31_t sqr_n(bb31_t s, uint32_t n)
  {
    ...
    while (n--) {
      ...
      asm("mul.lo.u32 %0, %2, %2; mul.hi.u32 %1, %2, %2;" : "=r"(tmp[0]), "=r"(tmp[1]) : "r"(s.val));
      asm("mul.lo.u32 %0, %1, %2;" : "=r"(red) : "r"(tmp[0]), "r"(M));
      asm("mad.lo.cc.u32 %0, %2, %3, %0; madc.hi.u32 %1, %2, %3, %4;"
          : "+r"(tmp[0]), "=r"(s.val)
          : "r"(red), "r"(MOD), "r"(tmp[1]));
...
public:
  ...
  friend inline bb31_t operator/(bb31_t a, bb31_t b) { return a * b.reciprocal(); }
  ...
  inline bb31_t dbl() const { return *this + *this; }

  inline bb31_t halve() const { return *this * bb31_t(2).reciprocal(); }

  inline bb31_t mul_2exp_neg_n(uint32_t n)
  {
    assert(n < 33);

    uint32_t two_exp_neg_n = 1 << (32 - n);
    return (*this * bb31_t(two_exp_neg_n));
  }
};
...
public:
  ...
  inline bb31_t reciprocal() const
  {
    ...
    bb31_t p1000010010000100100001001 = p1000000010000000100000001 * p10000000100000001000;
    bb31_t p10000000100000001000000010 = p1000000010000000100000001.square();
    bb31_t p11000010110000101100001011 = p10000000100000001000000010 * p1000010010000100100001001;
    bb31_t p100000001000000010000000100 = p10000000100000001000000010.square();
    bb31_t p111000011110000111100001111 = p100000001000000010000000100 * p11000010110000101100001011;
    bb31_t p1110000111100001111000011110000 = p111000011110000111100001111.exp_power_of_2(4);
    bb31_t p1110111111111111111111111111111 = p1110000111100001111000011110000 * p111000011110000111100001111;
