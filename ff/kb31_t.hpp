...
public:
  ...
  static const uint32_t RR = 0x37e1330bu;
  ...
  static const uint32_t TOP_BITS = 7;
  ...
  inline kb31_t operator-() { return zero() - *this; }
...
public:
  ...
  template <size_t T>
  static inline kb31_t dot_product(kb31_t a0, kb31_t b0, const kb31_t a[T - 1], const kb31_t* b, size_t stride_b = 1)
  {
    uint32_t acc[2];
    size_t i = 0;
...
public:
  ...
  {
    kb31_t p11, p1100, p1111, p110000, p111111, p1111110000, p1111111111, ret = *this;

    p11 = sqr_n_mul(ret, 1, ret); // ret^3
    p1100 = sqr_n(p11, 2);        // (ret^3)^(2^2) = ret^12
    p1111 = p1100 * p11;          // ret^12 * ret^3 = ret^15
    // (ret^12)^(2^2) * ret^15 = ret^48 * ret^15 = ret^63
    p111111 = sqr_n_mul(p1100, 2, p1111);
    p1111110000 = sqr_n(p111111, 4);   // (ret^63)^(2^4) = ret^1008
    p1111111111 = p1111110000 * p1111; // ret^1008 * ret^15 = ret^1023
    ret = p1111111111 * p1111110000;   // ret^1023 * ret^1008 = ret^2031
    // (ret^2031)^(2^10) * ret^1023 = ret^2080767
    ret = sqr_n_mul(ret, 10, p1111111111);
    // (ret^2080767)^(2^10) * ret^1023 = ret^2130706431
    ret = sqr_n_mul(ret, 10, p1111111111);
  ...
  {
    assert(false && "heptaroot() is not ready!");
    return *this;
  ...
  inline kb31_t dbl() const { return *this + *this; }

  inline kb31_t halve() const { return *this * kb31_t(2).reciprocal(); }

  inline kb31_t mul_2exp_neg_n(uint32_t n)
  {
    assert(n < 33);

    uint32_t two_exp_neg_n = 1 << (32 - n);
    return (*this * kb31_t(two_exp_neg_n));
  }
};
