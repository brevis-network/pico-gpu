...
namespace pico_gpu::ed_decompress {
  using namespace params_ed25519;
  using namespace edwards_op;

  template <class F>
  __global__ void edwards_decompress_kernel(
    ...
    F* byte_trace)
  {
    ...
    EdDecompressCols<F, NumLimbs, NumWitnesses>* col =
      reinterpret_cast<EdDecompressCols<F, NumLimbs, NumWitnesses>*>(&trace_matrix[idx * num_cols]);

    // Process event and populate row.
    ed_decompress_event_to_row<F, NumWords, NumLimbs, NumWitnesses, NumBitsPerLimb, WitnessOffset>(
