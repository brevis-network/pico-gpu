...
namespace pico_gpu::precompile_fp2_add_sub_bls12381 {
  using namespace ::pico_gpu::fp2_op;
  using namespace ::pico_gpu::params_bls12381;

  using Fp2AddSubEvent_t = Fp2AddSubEvent<NumWords>;

  template <class F>
  __global__ void fp2_add_sub_kernel(
    const Fp2AddSubEvent_t* events,
    const size_t count,
    F* trace_matrix,
    F* byte_trace,
    ...
    const size_t num_rows)
  {
    ...
    Fp2AddSubCols<F, NumWords, NumLimbs, NumWitnesses>* col =
      reinterpret_cast<Fp2AddSubCols<F, NumWords, NumLimbs, NumWitnesses>*>(&trace_matrix[idx * num_cols]);

    add_sub_event_to_row<F, NumWords, NumLimbs, NumWitnesses, NumBitsPerLimb, WitnessOffset>(
